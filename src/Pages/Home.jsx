...
export default function Home() {
  ...
  return (
    <OrderContext.Provider value={{ orders, setOrders }}>
      <div className="d-flex flex-column min-vh-100">
        {/* Top Navbar */}
        <Navbar bg="dark" variant="dark" expand="lg" className="px-3 py-2 animate__animated animate__fadeInDown">
          <Navbar.Brand className="d-flex align-items-center">
            <Image
              src="/logo.png"
              roundedCircle
              ...
              className="me-2"
            />
            <span className="fs-5">Delicious Restaurant</span>
          </Navbar.Brand>
          <Navbar.Toggle
        ...
        <div className="d-flex flex-grow-1">
          <Nav
            className="flex-column bg-light p-3 d-none d-md-block"
            style={{ width: '220px', minHeight: '100%' }}
            variant="pills"
            activeKey={location.pathname}
          >
            {sidebarItems.map(({ to, icon, text }) => (
              <Nav.Link
                key={to}
                as={NavLink}
                to={to}
        ...
        {/* Footer */}
        <footer className="bg-dark text-light text-center py-4 animate__animated animate__fadeInUp mt-auto">
          <Container>
            <div className="mb-1">📍 Address: Food Street, Vishakapatanam</div>
            <div className="mb-1">📞 Contact: 1234567890</div>
            <div className="mb-1">📧 Email: admin@deliciousrestaurant.com</div>
